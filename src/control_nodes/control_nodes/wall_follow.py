...
class WallFollow(Node):
    ...
    def get_error(self, range_data, angle_info):
        ...
        # angle_info['target_angle'] = np.radians(0) #self.angle_b
        # dist_b = self.get_range(range_data, angle_info)
        
        # angle_info['target_angle'] =np.radians() #self.angle_a
        # dist_a = self.get_range(range_data, angle_info)

        # # theta is the angle between the two rays
        # theta = self.angle_b - self.angle_a
        min_array,max_array=[],[]
        for deg in range(0, 20, 1):   # 0, 45, 90, ..., 315
            
            
            if deg<10:
                target_angle = np.deg2rad(310+deg)
            
            else:
                target_angle=np.deg2rad(255+deg)

            # Convert angle → index
            idx = int((target_angle - angle_info['min']) / angle_info['inc'])

            # Bounds check
            if idx < 0 or idx >= len(range_data):
                print(f"{target_angle:3d}° : out of range")
                continue
            ...
            if deg<10:
                if len(min_array)==0:
                    min_array.append([dist,target_angle])

                elif dist > min_array[0][0]:
                    min_array[0]=[dist,target_angle]

            else:
                if len(max_array)==0:
                    max_array.append([dist,target_angle])

                elif dist > max_array[0][0]:
                    max_array[0]=[dist,target_angle]


      

        dist_b=(min_array[0][0])

        dist_a=(max_array[0][0])

        theta=np.radians(max_array[0][1]-min_array[0][1])
    ...
    def scan_callback(self, msg):
        ...
        # for deg in range(0, 360, 1):   # 0, 45, 90, ..., 315
        #     target_angle = np.deg2rad(deg)
        ...
        #     if np.isnan(dist) or np.isinf(dist):
        #         print(f"{deg:3d}° : no return")
        #     else:
        #         print(f"{deg:3d}° : {dist:.2f} m")
        #     # 1. Calculate Error
