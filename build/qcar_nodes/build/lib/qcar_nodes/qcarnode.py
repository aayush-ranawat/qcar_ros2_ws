...
class QCarNode(Node):
	...
	def IMU_callback(self):
		...
		stateIMU.angular_velocity.x    = float(dataIMU[3])# 0
		stateIMU.angular_velocity.y    = float(dataIMU[4])# 1
		stateIMU.angular_velocity.z    = float(dataIMU[5])# 2
		stateIMU.linear_acceleration.x = float(dataIMU[0])# 3
		stateIMU.linear_acceleration.y = float(dataIMU[1])# 4
		stateIMU.linear_acceleration.z = float(dataIMU[2])# 5
